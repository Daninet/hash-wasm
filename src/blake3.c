...
static __inline__ uint32_t counter_low(uint64_t counter) { return (uint32_t)counter; }
...
static __inline__ void g(uint32_t *state, size_t a, size_t b, size_t c, size_t d,
...
static __inline__ void round_fn(uint32_t state[16], const uint32_t *msg, size_t round) {
...
static __inline__ void load_key_words(const uint8_t key[BLAKE3_KEY_LEN],
                           uint32_t key_words[8]) {
  memcpy32(key_words, key);
}

static __inline__ void compress_pre(uint32_t state[16], const uint32_t cv[8],
...
static __inline__ void chunk_state_init(blake3_chunk_state *self, const uint32_t key[8],
                             uint8_t flags) {
...
static __inline__ void chunk_state_reset(blake3_chunk_state *self, const uint32_t key[8],
                              uint64_t chunk_counter) {
  memcpy32(self->cv, key);
...
static __inline__ size_t chunk_state_fill_buf(blake3_chunk_state *self,
...
static __inline__ output_t make_output(const uint32_t input_cv[8],
                            const uint8_t block[BLAKE3_BLOCK_LEN],
                            uint8_t block_len, uint64_t counter,
                            uint8_t flags) {
  output_t ret;
...
static __inline__ void output_root_bytes(const output_t *self, uint64_t seek, uint8_t *out,
                              size_t out_len) {
...
static __inline__ void chunk_state_update(blake3_chunk_state *self, const uint8_t *input,
                               size_t input_len) {
  if (self->buf_len > 0) {
...
static __inline__ output_t parent_output(const uint8_t block[BLAKE3_BLOCK_LEN],
...
static __inline__ void hash_one_portable(const uint8_t *input, size_t blocks,
...
static __inline__ size_t compress_chunks_parallel(const uint8_t *input, size_t input_len,
...
static __inline__ size_t compress_parents_parallel(const uint8_t *child_chaining_values,
  ...
  if (num_chaining_values > 2 * parents_array_len) {
    memcpy32(&out[parents_array_len * BLAKE3_OUT_LEN], &child_chaining_values[2 * parents_array_len * BLAKE3_OUT_LEN]);
...
static __inline__ void hasher_init_base(blake3_hasher *self, const uint32_t key[8],
                             uint8_t flags) {
...
static __inline__ void hasher_push_cv(blake3_hasher *self, uint8_t new_cv[BLAKE3_OUT_LEN],
                           uint64_t chunk_counter) {
