...
const hashMultiple = (...params: Parameters<typeof hash>) => {
  ...
  return parr.map((p) => hash(...p as Parameters<typeof hash>));
};

test('argon2d', () => {
  expect(
    hash('a', 'abcdefgh', 1, 2, 16, 16, 'hex', 'd'),
  ).toBe('77ab7ab1e8b3a4c3610327502709b131');

  expect(
    hash('text demo', '123456789', 7, 20, 56, 32, 'hex', 'd'),
  ).toBe('4470e3dea5a56405b579940a0da1be1f0b93dde207abf29a97fa24ea80ec1ad7');

  expect(
    hash('text demo', '123456789', 2, 1, 32, 32, 'hex', 'd'),
  ).toBe('e606633e6ce0148e7397c48fae9ce69e75c2dbb94fa87ac70b620d36855ca317');

  expect(
    hash('abc', '1234567812345678', 1, 1, 8, 4, 'hex', 'd'),
  ).toBe('1fe14ab8');

  expect(
    hash('abc', '12345678', 3, 5, 31, 18, 'hex', 'd'),
  ).toBe('1a4a99591876ae1f0fa0a664c15cb727377e');
});

test('argon2id', () => {
  expect(
    hash('a', 'abcdefgh', 1, 2, 16, 16, 'hex', 'id'),
  ).toBe('f94aa50873d67fdd589d6774b87c0634');

  expect(
    hash('text demo', '123456789', 7, 20, 56, 32, 'hex', 'id'),
  ).toBe('ec2f7a502b4bfe7dc758c4c5120c7420830d42efdc7a78971743649b30cafb15');

  expect(
    hash('text demo', '123456789', 2, 1, 64, 32, 'hex', 'id'),
  ).toBe('b3f9d902f65bd329cf0810c78b19c746b6f46fbb8243647a8942ab83b6850d47');

  expect(
    hash('abc', '1234567812345678', 1, 1, 8, 4, 'hex', 'id'),
  ).toBe('ffde3f6a');

  expect(
    hash('abc', '12345678', 3, 5, 31, 18, 'hex', 'id'),
  ).toBe('4e37e561150881c66096b17bc6a326e37e87');
});

test('argon2i', () => {
  expect(
    hash('a', 'abcdefgh', 1, 2, 16, 16, 'hex', 'i'),
  ).toBe('2852426eb498671a10f2a91185baec84');

  expect(
    hash('text demo', '123456789', 7, 20, 56, 32, 'hex', 'i'),
  ).toBe('d8a577f24c319bfae90556a30851ac5e031c7f6e752fb6caf05471a26ce061bb');

  expect(
    hash('text demo', '123456789', 2, 1, 64, 32, 'hex', 'i'),
  ).toBe('d88b0eafcd02b94b28f77246dda8c1a47533cfc7e6bd948edb5eca27fdbb7dc9');

  expect(
    hash('abc', '1234567812345678', 1, 1, 8, 4, 'hex', 'i'),
  ).toBe('ced82c90');

  expect(
    hash('abc', '12345678', 3, 5, 31, 18, 'hex', 'i'),
  ).toBe('c0efcdc71934adfa47d3e48cb82679899b21');
});

test('others', () => {
  expect(
    hashMultiple('password', 'somesalt', 1, 1, 64, 24, 'hex'),
  ).toMatchObject([
  ...
  expect(
    hashMultiple('password', 'somesalt', 1, 2, 64, 24, 'hex'),
  ).toMatchObject([
  ...
  expect(
    hashMultiple('password', 'somesalt', 2, 2, 64, 24, 'hex'),
  ).toMatchObject([
  ...
  expect(
    hashMultiple('password', 'somesalt', 2, 3, 256, 24, 'hex'),
  ).toMatchObject([
  ...
  expect(
    hashMultiple('password', 'somesalt', 4, 4, 4096, 24, 'hex'),
  ).toMatchObject([
  ...
  expect(
    hashMultiple('password', 'somesalt', 8, 4, 1024, 24, 'hex'),
  ).toMatchObject([
  ...
  expect(
    hashMultiple('password', 'somesalt', 3, 2, 64, 24, 'hex'),
  ).toMatchObject([
  ...
  expect(
    hashMultiple('password', 'somesalt', 6, 3, 1024, 24, 'hex'),
  ).toMatchObject([
  ...
  expect(
    hashMultiple('qwe', 'somesalt123', 1, 1, 8, 7, 'hex'),
  ).toMatchObject([
    '688c5ac265ee78',
    '3af555cede3a50',
    'f7557d529ee588',
  ...
  expect(
    hashMultiple('qwe', 'somesalt123', 1, 1, 9, 7, 'hex'),
  ).toMatchObject([
    '774cf6adb35c48',
    '68e9964c622ea7',
    'aca65924f01fb9',
  ...
  expect(
    hashMultiple('qwe', 'somesalt123', 5, 7, 41, 15, 'hex'),
  ).toMatchObject([
  ...
  expect(
    hashMultiple('qwe', 'somesalt123', 5, 7, 139, 15, 'hex'),
  ).toMatchObject([
  ...
  expect(
    hashMultiple('qwe', 'somesalt123', 1, 1, 32, 71, 'hex'),
  ).toMatchObject([
...
test('binary input', () => {
  expect(
    hashMultiple(Buffer.from([0]), Buffer.from([0, 0, 0, 0, 0, 0, 0, 0]), 5, 17, 53, 16, 'hex'),
  ).toMatchObject([
  ...
  expect(
    hashMultiple(Buffer.from([255, 255]), Buffer.from([255, 255, 255, 255, 255, 255, 255, 255]), 5, 17, 53, 16, 'hex'),
  ).toMatchObject([
...
test('encoded output', () => {
  expect(
    hashMultiple('qwe', 'somesalt123', 5, 7, 139, 15, 'encoded'),
  ).toMatchObject([
  ...
  expect(
    hashMultiple('qwe', 'somesalt1234', 5, 7, 139, 14, 'encoded'),
  ).toMatchObject([
  ...
  expect(
    hashMultiple('qwer', 'somesalt12345', 1, 3, 15, 13, 'encoded'),
  ).toMatchObject([
...
test('binary output', () => {
  expect(
    ArrayBuffer.isView(
      hash('a', 'abcdefgh', 1, 2, 16, 16, 'binary', 'd'),
    ),
  ).toBe(true);

  expect(
    hashMultiple('qwe', 'somesalt123', 5, 7, 41, 15, 'binary'),
  ).toMatchObject([
    new Uint8Array(
...
test('longer calculations', () => {
  expect(
    hashMultiple('password', 'somesalt123', 11, 27, 13921, 16, 'hex'),
  ).toMatchObject([
  ...
  expect(
    hashMultiple('password', 'somesalt123', 3, 4500, 99, 16, 'hex'),
  ).toMatchObject([
  ...
  expect(
    hashMultiple('password', 'somesalt123', 1, 2, 250000, 16, 'hex'),
  ).toMatchObject([
